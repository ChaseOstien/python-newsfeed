...
db = Session() # establishes a temporary database connection to perform CRUD operations. Neccesary to perform CRUD operations.

# insert users
db.add_all([ # This code only preps the SQL queries and does not change the database itself
...
db.commit() # you must call this in order to actually change the database.
...
db.close() # Call this after all of your database transactions to close the session connection.